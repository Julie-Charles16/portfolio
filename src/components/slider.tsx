...
<div className="btns flex justify-center gap-5 mt-[clamp(1rem,5vh,3rem)]">
  <FontAwesomeIcon
    onClick={prev}
    icon={faChevronLeft}
    size="1x"
    className="btn w-8 h-8 sm:w-10 sm:h-10 flex justify-center items-center rounded-full transition-all bg-black/20 dark:bg-white/20 border border-black/25 dark:border-white/25 shadow-md dark:shadow-md hover:translate-y-[-3px] hover:scale-105 hover:bg-black/30 dark:hover:bg-white/30"
  />
  <FontAwesomeIcon
    onClick={next}
    icon={faChevronRight}
...
function SliderContent({ title, desc, image, tech, github, demo }: SliderContentProps) {
  return (
    <div className="sliderContent flex flex-col justify-between items-center h-full p-4 text-center overflow-hidden">
      {image && <img src={image} alt={title} className="projectImage w-full max-h-[180px] object-cover rounded-lg" />}
      <h2 className="font-bold text-lg sm:text-xl md:text-2xl">{title}</h2>
      <p className="text-sm sm:text-base md:text-lg leading-relaxed overflow-hidden text-ellipsis max-h-[4.5em]">{desc}</p>

      <div className="techStack flex flex-wrap gap-2 mt-2">
        {tech?.map((t, i) => (
          <span key={i} className="tech bg-white/20 px-2 py-1 rounded-md text-xs">{t}</span>
        ))}
      </div>

      <div className="links flex gap-3 mt-3">
        {github && (
          <a href={github} target="_blank" rel="noopener noreferrer" className="border border-white px-3 py-1 rounded-md font-bold hover:bg-white hover:text-black transition">
            Code
          </a>
        )}
        {demo && (
          <a href={demo} target="_blank" rel="noopener noreferrer" className="border border-white px-3 py-1 rounded-md font-bold hover:bg-white hover:text-black transition">
            Demo
          </a>
        )}
      </div>
